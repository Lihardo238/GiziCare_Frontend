...
import { useRouter } from "next/navigation"; // <-- PERUBAHAN: Import useRouter untuk navigasi
import React, { useState, useEffect, useRef, useMemo } from "react";

// --- PERUBAHAN: Tipe data untuk setiap perintah ---
type Command = {
  name: string;
  keywords: string[];
  path?: string;
...
// --- PERUBAHAN: Daftar perintah yang tersedia ---
// Ini adalah "otak" dari command palette kita. Anda bisa menambahkannya dengan mudah.
const availableCommands: Command[] = [
  {
    name: "Dashboard",
    keywords: ["beranda", "utama", "dashboard"],
    path: "/dashboard",
  },
  {
    name: "Catatan Harian (Activity Diary)",
    keywords: ["aktivitas", "catatan", "diary", "olahraga", "log"],
    path: "/activity-diary",
  },
  {
    name: "Catatan Makanan (Food Diary)",
    keywords: ["makanan", "nutrisi", "kalori", "log makanan"],
    path: "/food-diary",
  },
  {
    name: "Manajemen Profil",
    keywords: ["pengguna", "akun", "profile", "ubah profil", "personalisasi"],
    path: "/profile/edit",
  },
  {
  name: "Riwayat Aktivitas",
  keywords: ["history", "riwayat", "aktivitas", "log", "delete"],
...
const AppHeader: React.FC = () => {
  ...
  const searchContainerRef = useRef<HTMLDivElement>(null); // Ref untuk container search
  const router = useRouter(); // Hook untuk navigasi

  // --- PERUBAHAN: State untuk fungsionalitas command palette ---
  const [searchTerm, setSearchTerm] = useState("");
  const [isResultsVisible, setResultsVisible] = useState(false);

  // --- PERUBAHAN: Logika filter menggunakan useMemo untuk performa ---
  const filteredCommands = useMemo(() => {
    ...
    return availableCommands.filter(
      (command) =>
        ...
        command.keywords.some((kw) => kw.includes(lowerCaseSearchTerm))
    );
  }, [searchTerm]);
  ...
  // --- PERUBAHAN: Fungsi untuk mengeksekusi perintah ---
  const handleCommandSelect = (command: Command) => {
    ...
    setSearchTerm(""); // Kosongkan input setelah dieksekusi
    setResultsVisible(false); // Sembunyikan hasil
    inputRef.current?.blur(); // Hilangkan fokus dari input
  };

  // Hook untuk shortcut ⌘K (sudah benar)
  useEffect(() => {
  ...
  // --- PERUBAHAN: Hook untuk menutup hasil pencarian saat klik di luar ---
  useEffect(() => {
    ...
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-50 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          {/* --- PERUBAHAN: Container untuk form dan hasil pencarian --- */}
          <div className="hidden lg:block relative" ref={searchContainerRef}>
            ...
            {/* --- PERUBAHAN: Dropdown untuk menampilkan hasil --- */}
            {isResultsVisible && filteredCommands.length > 0 && (
              <div className="absolute top-full mt-2 w-full rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
                <ul className="py-2">
                  {filteredCommands.map((command) => (
                    <li
                      key={command.name}
                      className="px-4 py-2 text-sm text-gray-700 cursor-pointer hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"
                      // Gunakan onMouseDown untuk "mengalahkan" onBlur dari input
                      onMouseDown={() => handleCommandSelect(command)}
                    >
                      {command.name}
                    </li>
                  ))}
        ...
        >
          <div className="flex items-right gap-0.5 2xsm:gap-0.5">
